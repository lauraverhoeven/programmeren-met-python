...
# a function to calculate efficiency


def efficiency(data):
    a = data.loc[:, "VermedenEmissie_5"]
    b = data.loc[:, "BrutoEindverbruik_1"]
    # converting strings to integers
    int_a = int(a)
    int_b = int(b)
    yearsefficiency = (int_a/int_b)
    return yearsefficiency
# a function that makes a barchart.
...
def barchart(data, efficiency):
    ...
    while Repeat:
        ...
        list2017.append(d)
        yearbar = int(input(
            "Of which year do you want to see the efficiency of different renewable energy sources? 2017 of 2018?"))
        if yearbar == 2017:
            list2 = []
            for i in list2017:
                list2.append(efficiency(data))
            Renewable_frame = pd.DataFrame(
                {'Energy source': list0, 'Efficiency': list2})
            Efficiencybars = Renewable_frame.plot.bar(
                x='Energy source', y='Efficiency', title='Efficiency:Avoided Emission/Consumption (in Kton/TJ)', rot=0)
            plt.show()
        elif yearbar == 2018:
            list2 = []
            for i in list2018:
        ...
        if Quit == "y":
            Repeat = False
# a function that makes a line chart
...
def linechart(data, efficiency):
    ...
    while repeat:
        try:
            ...
            year1 = int(
                input('Which year do you want the line graph to start with?'))
            year2 = int(
                input('Which year do you want the line graph to end with?'))
            ...
            if 1989 <= year1 <= 2018 and 1989 <= year2 <= 2018 and year2 > year1:
                ...
                sustainablestructure = pd.DataFrame({"Waterforce": wateryearlist, "Wind": windyearlist,
                                                     "Sun": sunyearlist, "Earth energy": earthyearlist, "Biomass": bioyearlist}, index=yearslist)
                ...
                print(
                    "You typed the wrong number. Note that the first number should be smaller than the second. Please try again.")
        except Exception:
            print("You must type a number/ numbers")
        Quit = input("Do you want to quit? y/n")
        if Quit == "y":
            repeat = False
